#!/usr/bin/env python
# https://github.com/ZeyadZanaty/cs224n-assignments/tree/master/Assignment-2 참고
...
def naiveSoftmaxLossAndGradient(
    ...
    _softmax[outsideWordIdx] -= 1 # y_hat - y
    gradCenterVec = np.dot(outsideVectors.T, _softmax) # U 내적 (y_hat - y)

    gradOutsideVecs = np.dot(np.expand_dims(_softmax, axis=1), np.expand_dims(centerWordVec, axis=0)) # (y_hat - y) 내적 v_c
    # gradOutsideVecs = np.dot(_softmax, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    dot_one = outsideVectors[outsideWordIdx].T.dot(centerWordVec) # u_o . v_c
    neg_samples = outsideVectors[negSampleWordIndices] # K개의 단어들
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        # currentCenterWord와, outsidewords에서 단어를 하나하나 받아서 인덱스로 변환시켜 인풋으로 넣고, 전체 outside vector인 U도 인풋으로 받아서
        # 출력으로 loss, V의 gradient, U의 gradient를 내뱉는다.
